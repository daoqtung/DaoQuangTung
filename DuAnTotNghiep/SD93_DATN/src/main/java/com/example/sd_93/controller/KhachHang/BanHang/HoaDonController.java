package com.example.sd_93.controller.KhachHang.BanHang;

import com.example.sd_93.controller.Momo.MomoModel;
import com.example.sd_93.controller.Momo.ResultMoMo;
import com.example.sd_93.controller.Utils.Constant;
import com.example.sd_93.controller.Utils.Decode;
import com.example.sd_93.model.*;
import com.example.sd_93.repository.BanHang.GioHangChiTietRepository;
...
import com.example.sd_93.repository.KhachHangRepository;
import com.example.sd_93.repository.SanPham.AllGiayTheThao.MauSacRepository;
...
import com.example.sd_93.repository.SanPham.GiayTheThao.GiayTheThaoRepository;
import com.example.sd_93.service.HoaDon.HoaDonService;
import com.example.sd_93.service.HoaDon.HoaDonServiceImpl;
import com.example.sd_93.service.impl.ChuongTrinhGiamGiaHoaDonImpl;
import com.fasterxml.jackson.core.JsonProcessingException;
import com.example.sd_93.repository.BanHang.GioHangRepository;
import com.example.sd_93.service.ViTien.Impl.giaoDichViChiTietServiceImpl;
import com.example.sd_93.service.ViTien.Impl.viTienServiceImpl;
...
//thay thế
                List<UUID> hoaDonChiTietIds = hoaDonChiTietList.stream()
                    ...
                    if ("diaChiCu".equals(diaChiChon)) {
                        ...
                        if ("cash".equals(hinhThucThanhToan)) {
                            ...
                            if (hoaDon != null) {
                                ...
                                if (ship.matches("\\d*\\.?\\d*")) {
                                    ...
                                    String giamGia = request.getParameter("giamGia"); // Lấy giá trị giảm giá
                                    BigDecimal giamGiaBigDecimal = (giamGia != null && !giamGia.isEmpty())
                                            ? new BigDecimal(giamGia)
                                            : BigDecimal.ZERO;

                                    System.out.println("Giá trị giảm giá: " + giamGiaBigDecimal);

                                    // Tổng tiền sau khi áp dụng giảm giá và thêm phí ship
                                    BigDecimal thanhTienSauGiamGia = thanhTienBigDecimal.subtract(giamGiaBigDecimal);
                                    BigDecimal thanhTienSauShip = thanhTienSauGiamGia.add(shipBigDecimal);
                        ...
                        if ("vi".equals(hinhThucThanhToan)) {
                            ...
                            if (viTien.getThanhTien().intValue() < hoaDon.getThanhTien().intValue()) {
                                ...
                                attributes.addFlashAttribute("erVi", "Xin lỗi hiện tại số dư trong ví của quý khách không đủ để thanh toán !");
                                return "redirect:/nguoiDung/HoaDon/" + hoaDonId;
                                ...
                                if (hoaDon != null) {
                                    ...
                                    if (ship.matches("\\d*\\.?\\d*")) {
                                        ...
                                        BigDecimal tongTienVi = viTien.getThanhTien().subtract(thanhTienSauShip);
...
//
                            if (hoaDon != null) {
                                ...
                                String phiShipMoi = request.getParameter("ship");
                                ...
                                jsonRequest.setOrderInfo("Thanh toán sản phẩm cửa hàng Easy-Shop");
                                jsonRequest.setRequestId(orderId);
                                jsonRequest.setOrderType(Constant.orderType);
                                jsonRequest.setRequestType(Constant.requestType);
                                jsonRequest.setTransId("1");
...
//                                        BigDecimal tongTienVi = viTien.getThanhTien().subtract(hoaDon.getThanhTien());
    ...
    public ModelAndView hoaDonChoThanhToan(
        ...
        KhachHang khachHang = khachHangRepository.findByMaKhachHang(ma);
        Page<HoaDon> page = hoaDonServiceImpl.listHoaDonFindByKhachHangAndTrangThai(khachHang.getId(), 0, pageNo, 10);
    ...
    public ModelAndView hoaDonChoXacNhan(
        ...
        List<HoaDon> sortedList = hoaDonServiceImpl.listHoaDonFindByKhachHangAndTrangThai(khachHang.getId(), 1, pageNo, 10)
                .getContent()
                .stream()
    ...
    public ModelAndView choDongGoiPhiaKhachHang(
        ...
        Page<HoaDon> page = hoaDonServiceImpl.listHoaDonFindByKhachHangAndTrangThai(khachHang.getId(), 2, pageNo, 10);
    ...
    public ModelAndView dangGiaoHangBenPhiaKhachHang(
        ...
        KhachHang khachHang = khachHangRepository.findByMaKhachHang(ma);
        Page<HoaDon> page = hoaDonServiceImpl.listHoaDonFindByKhachHangAndTrangThai(khachHang.getId(), 3, pageNo, 10);
    ...
    public ModelAndView giaoHangThanhCong(
        ...
        KhachHang khachHang = khachHangRepository.findByMaKhachHang(ma);
        Page<HoaDon> page = hoaDonServiceImpl.listHoaDonFindByKhachHangAndTrangThai(khachHang.getId(), 4, pageNo, 10);
    ...
    public ModelAndView hoaDonHuy(@RequestParam(value = "pageNo", required = false, defaultValue = "0") Integer pageNo,
        ...
        KhachHang khachHang = khachHangRepository.findByMaKhachHang(ma);
        Page<HoaDon> page = hoaDonServiceImpl.listHoaDonFindByKhachHangAndTrangThai(khachHang.getId(), 5, pageNo, 10);
