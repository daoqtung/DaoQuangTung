...
import com.example.sd_93.repository.SanPham.AllGiayTheThao.TrongLuongRepository;
import com.example.sd_93.service.admin.TrongLuongService;
...
public class TrongLuongController {
    ...
    public String hienThi(Model model, @RequestParam(name = "num",defaultValue = "0") Integer num) {
        Page<TrongLuong> trongLuongPage = trongLuongRepository.findAll(PageRequest.of(num,5));
    ...
    public String detail(@PathVariable UUID id, Model model){
        ...
        // Format date before sending it to view
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        LocalDate ngayTao = LocalDate.parse(trongLuong.getNgayTao());
        LocalDate ngaySua = LocalDate.parse(trongLuong.getNgaySua());

        trongLuong.setNgayTao(ngayTao.format(formatter));
        trongLuong.setNgaySua(ngaySua.format(formatter));
    ...
    public String update(@PathVariable("id") UUID id, @ModelAttribute("trongLuong") @Valid TrongLuong trongLuong,
                         BindingResult bindingResult,
                         Model model) {
        if(bindingResult.hasErrors()){
            model.addAttribute("trongLuong",trongLuong);
            ...
            TrongLuong existingTrongLuong = trongLuongService.getOne(id);
            trongLuong.setNgayTao(existingTrongLuong.getNgayTao());

            LocalDate today = LocalDate.now();
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            String formattedDate = today.format(formatter);
            trongLuong.setNgaySua(formattedDate);
            trongLuong.setId(id);
            trongLuongService.save(trongLuong);
            return "redirect:/trongLuong/hien-thi";
        }
