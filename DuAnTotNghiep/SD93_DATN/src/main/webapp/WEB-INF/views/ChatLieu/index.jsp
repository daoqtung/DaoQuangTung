...
<br>
    <div class="d-flex justify-content-between mb-3">
        <div class="search-form">
            <form action="/chatLieu/search" method="get" class="form-inline">
                <input type="text" class="form-control mr-2" name="tenChatLieu" placeholder="Tìm kiếm chất liệu" style="width: 300px;">
                <button type="submit" class="btn btn-info"><i class="fas fa-search"></i> Tìm kiếm</button>
            </form>
        </div>

        <div class="filter-form">
            <form action="/chatLieu/filter" method="get" class="form-inline">
    ...
    <div class="text-left mb-3">
        <a href="/chatLieu/view-add" class="btn btn-primary btn-custom">
            <i class="fas fa-plus"></i> Thêm Mới Chất Liệu
        </a>
    </div>
        ...
        <table class="table table-striped table-bordered">
            ...
            <c:forEach items="${list}" var="d" varStatus="i">
                <tr>
                    ...
                    <td>${d.trangThai == 1 ? "✔" : "✖"}</td>
                    <td>
                        <a href="/chatLieu/detail/${d.id}" class="btn btn-info btn-custom-size">
                            <i class="fas fa-edit"></i> Sửa Chất Liệu Giày
                        </a>
                    </td>
                </tr>
            </c:forEach>
    ...
    <div class="d-flex justify-content-center">
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item">
                    <c:if test="${next > 0}">
                        <a class="page-link" href="/chatLieu/hien-thi?num=${next - 1}">
                ...
                <c:forEach begin="1" end="${totalPages}" varStatus="loop">
                    <li class="page-item ${loop.index == next + 1 ? 'active' : ''}">
                        <a class="page-link" href="/chatLieu/hien-thi?num=${loop.index - 1}">${loop.index}</a>
                    </li>
                </c:forEach>
                <li class="page-item">
