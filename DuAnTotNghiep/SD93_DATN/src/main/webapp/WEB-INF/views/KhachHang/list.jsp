...
<style>
    ...
    input[type="text"] {
        ...
        transition: box-shadow 0.3s ease; /* Hiệu ứng chuyển tiếp cho đổ bóng */
    }

    input[type="text"]:focus {
        border-color: #007bff; /* Màu viền khi focus */
        outline: none; /* Ẩn outline */
        box-shadow: 0 4px 8px rgba(0, 123, 255, 0.3); /* Đổ bóng khi focus */
    }

    .input-password {
        width: 110px; /* Chiều rộng mong muốn */
        height: 35px; /* Chiều cao */
        border-radius: 5px; /* Bo góc */
        border: 1px solid #ced4da; /* Viền */
        padding: 5px; /* Padding */
    ...
    <table class="table table-striped table-bordered">
        ...
        <c:forEach var="kh" items="${listKH}" varStatus="i">
            <tr class="product-item hide-row" data-tenKhachHang="${kh.tenKhachHang}" data-email="${kh.email}" data-soDienThoai="${kh.soDienThoai}" data-diaChi="${kh.diaChi}">
                ...
                <td>${kh.email}</td>
                <td><input type="password" value="${kh.matKhau}" class="input-password" readonly></td>
                <td>${kh.gioiTinh}</td>
                <td>${kh.ngaySinh}</td>
